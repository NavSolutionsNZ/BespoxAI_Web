...
#### 0.5. **(S28) BC15+/AL object source — content path proven, mechanism is expensive; versioning strategy needs to change**
**Status:** Investigated to a firm conclusion using a real test extension (`TestExtn1`). Parked as a build item (not urgent — Rich: "sideline gig only if in demand"), but no longer an open question — we now know exactly what it would take.

**Routing bug — fixed this session:** `app/api/requirements/[id]/fetch-objects/route.ts` was routing ANY `navProduct==='BC'` tenant (any version) into the finsql/C/AL export path, which doesn't exist past BC14 (Microsoft dropped the Windows client/C/SIDE in BC 2019 release wave 2). BC15+ tenants (e.g. the BC22 Cronus test container) hard-failed with a confusing "finsql.exe not found" instead of reaching the metadata-only fallback. Routing is now keyed off `tenant.navVersion` via a new `isModernAL()` helper.

**Object-source extraction — tested live against `TestExtn1` (`resourceExposurePolicy.allowDownloadingSource: true`), two mechanisms, two different outcomes:**
- **`Get-NavAppRuntimePackage -ShowMyCode $true` — dead end for this purpose.** Produces a package, but it is NOT a zip (confirmed by byte-scanning the file for the `PK\x03\x04` zip signature — not present anywhere). This cmdlet's own docs describe its purpose as enabling "debugging, recompiling, and running `Repair-NAVApp`" — it's a machine-to-machine artifact meant to feed back into another BC server, not a human-readable export. Do not build around this cmdlet for source export.
- **Extension Management page's "Download Source" button — this is the real mechanism, and it works.** Downloaded output for `TestExtn1` was a clean, standard zip (`app.json`, `.vscode/`, and `CustomerListExt.PageExt.al` with genuine readable AL source) — exactly the shape of data needed. BUT: captured via browser DevTools, the download is a two-step, session-scoped flow — `GET /BC/DynamicFileHandler.axd?form=118&sessionid=<company+session token>&type=File&fid=<per-request token>` — where `sessionid` and `fid` are minted server-side at the moment the "Download Source" action runs inside an authenticated web-client session. Not a stateless call BCAgent can replicate with `HttpWebRequest` the way every other BCAgent operation works today (OData, `New-NAVWebService`, etc.) — realistically requires driving the actual BC web client (e.g. headless browser automation via Playwright) to establish the session, trigger the action, and follow the resulting token. Meaningfully bigger and more fragile than anything else in BCAgent's architecture.

**Decision (Rich, this session):** don't build the browser-automation version now. Refocus product direction: **primary/fully-supported target stays NAV and Business Central up to BC14 (C/AL, OData-capable)** as originally planned. BC15+/AL support is best-effort/metadata-only, offered as a sideline only if a real customer needs it. If/when it's worth the engineering cost, the path above is proven and ready to build from — no more research needed, just the browser-automation implementation work.
