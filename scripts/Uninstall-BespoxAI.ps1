...
if ($cfSvc) {
    ...
    # Uninstall via cloudflared.exe if present, otherwise sc.exe
...
# Find PowerShell processes with BCAgent.ps1 in their command line
...
try {
    $netstat = netstat -ano | Select-String ":$AgentPort\s"
    if ($netstat) {
        foreach ($line in $netstat) {
            ...
            if ($pid -match '^\d+$' -and [int]$pid -gt 0) {
                ...
                if ($proc) {
                    Write-Warn "Port $AgentPort held by PID $pid ($($proc.Name)) — killing..."
...
if (-not (Test-Path $InstallRoot)) {
    ...
    if ($RemoveData) {
        ...
        # Preserve Deployments and Regression, remove everything else
        ...
        # Remove any loose files at root level
...
# ── Step 6: Verify nothing is holding the port ────────────────────────────────
...
try {
    ...
    $tcp.Close()
    Write-Warn "Port $AgentPort is STILL in use — something else may be listening. Check with: netstat -ano | findstr :$AgentPort"
} catch {
