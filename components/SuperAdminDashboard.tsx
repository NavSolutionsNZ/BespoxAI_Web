...
function RevBar({ pct, color, title }: { pct: number; color: string; title: string }) {
  if (pct <= 0) return null
  return <div style={{ width: pct+'%', background: color, height: '100%' }} title={title} />
}
...
function BillingCharts({ billing, onNavigate }: {
  ...
  const moDeposits = (billing.dev?.recentDeposits ?? []).filter(r => isThisMonth(r.paidAt))
  const moBalances = (billing.dev?.recentBalances ?? []).filter(r => isThisMonth(r.paidAt))
  const moReviews  = billing.reviews.list.filter(r => isThisMonth(r.paidAt))

  type ActivityRow = { label: string; key: string; count: number; value: string|null; pos: boolean }
  const activityRows: ActivityRow[] = [
    ...
    { label: 'Dev deposits',      key: 'deposits',      count: billing.dev?.thisMonth.deposits.count??0, value: (billing.dev?.thisMonth.deposits.revenueNZD??0) > 0 ? '+$'+(billing.dev?.thisMonth.deposits.revenueNZD??0).toLocaleString() : null, pos: true },
    { label: 'Dev balances',      key: 'balances',      count: billing.dev?.thisMonth.balances.count??0, value: (billing.dev?.thisMonth.balances.revenueNZD??0) > 0 ? '+$'+(billing.dev?.thisMonth.balances.revenueNZD??0).toLocaleString() : null, pos: true },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 24 }}>
      ...
      {/* ── Row 1: Revenue composition + Monthly activity ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 14 }}>
        ...
        <div style={{ background: 'var(--white)', border: '1px solid var(--fog)', borderRadius: 12, padding: '20px 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
            <div style={{ width: 104, height: 104, borderRadius: '50%', flexShrink: 0, background: 'conic-gradient(#0A5C46 0% '+subPct+'%, #C8952A '+subPct+'% '+(subPct+revPct)+'%, #1A9272 '+(subPct+revPct)+'% '+(subPct+revPct+devPct)+'%, var(--fog) '+(subPct+revPct+devPct)+'% 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                {activeTiers.length === 0 ? <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--fog)' }}>No active subscriptions</div> : null}
              </div>
              {(revFees > 0 || billing.reviews.allTime.count > 0) ? (
                <div style={{ borderTop: '1px solid var(--fog)', paddingTop: 10 }}>
                  ...
                  {billing.reviews.allTime.revenueNZD > 0 ? <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--slate)' }}>{'All-time: $'+billing.reviews.allTime.revenueNZD.toLocaleString()+' NZD'}</div> : null}
                </div>
              ) : null}
                ...
                <div style={{ borderTop: '1px solid var(--fog)', paddingTop: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--slate)' }}>{'Development ('+(( billing.dev?.thisMonth.deposits.count??0)+(billing.dev?.thisMonth.balances.count??0))+' payments this mo)'}</span>
                    </div>
                  ...
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--slate)', display: 'flex', gap: 12 }}>
                    {(billing.dev?.thisMonth.deposits.count??0) > 0 ? <span>{'Deposits: $'+(billing.dev?.thisMonth.deposits.revenueNZD??0).toLocaleString()}</span> : null}
                    {(billing.dev?.thisMonth.balances.count??0) > 0 ? <span>{'Balances: $'+(billing.dev?.thisMonth.balances.revenueNZD??0).toLocaleString()}</span> : null}
                    {(billing.dev?.allTime.totalNZD??0) > 0 ? <span style={{ marginLeft: 'auto' }}>{'All-time: $'+(billing.dev?.allTime.totalNZD??0).toLocaleString()+' NZD'}</span> : null}
                  </div>
                </div>
              ) : null}
            </div>
        ...
        <div style={{ background: 'var(--white)', border: '1px solid var(--fog)', borderRadius: 12, padding: '20px 24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {activityRows.map((row, i, arr) => {
              const isOpen    = financeDrill === row.key
              const hasDetail = row.count > 0
              return (
                <div key={row.key}>
                  <div
                    onClick={() => hasDetail ? setFinanceDrill(isOpen ? null : row.key) : undefined}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 0', borderBottom: (!isOpen && i < arr.length-1) ? '1px solid var(--fog)' : 'none', cursor: hasDetail ? 'pointer' : 'default' }}
                  >
                    <span style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: hasDetail ? 'var(--ink)' : 'var(--slate)' }}>{row.label}</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      {row.value ? <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: row.pos ? 'var(--forest)' : '#A32D2D' }}>{row.value}</span> : null}
                      ...
                      {hasDetail ? <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--slate)' }}>{isOpen ? '▲' : '▼'}</span> : null}
                    </div>
                  </div>
                  {isOpen ? (
                    <div style={{ background: 'rgba(10,92,70,0.03)', border: '1px solid var(--fog)', borderRadius: 8, margin: '4px 0 8px', overflow: 'hidden' }}>
                      ...
                      {row.key === 'reviews' ? moReviews.map((r, ri) => (
                        <div key={ri} onClick={() => r.id ? onNavigate('requirements', r.id) : undefined} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', borderBottom: ri < moReviews.length-1 ? '1px solid var(--fog)' : 'none', cursor: r.id ? 'pointer' : 'default' }}>
                          <div>
                      ...
                      {row.key === 'deposits' ? moDeposits.map((r, ri) => (
                        <div key={ri} onClick={() => r.id ? onNavigate('requirements', r.id) : undefined} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', borderBottom: ri < moDeposits.length-1 ? '1px solid var(--fog)' : 'none', cursor: r.id ? 'pointer' : 'default' }}>
                          <div>
                            <div style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--ink)', fontWeight: 500 }}>{r.title}</div>
                      ...
                      {row.key === 'balances' ? moBalances.map((r, ri) => (
                        <div key={ri} onClick={() => r.id ? onNavigate('requirements', r.id) : undefined} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', borderBottom: ri < moBalances.length-1 ? '1px solid var(--fog)' : 'none', cursor: r.id ? 'pointer' : 'default' }}>
                          <div>
                            <div style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--ink)', fontWeight: 500 }}>{r.title}</div>
                            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--slate)' }}>{r.tenant+' · '+relativeTime(r.paidAt)}</div>
                          </div>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#0A5C46', fontWeight: 600 }}>{'$'+r.amountNZD.toLocaleString()}</span>
                        </div>
                      )) : null}
                      {row.key === 'upgrades' ? billing.upgrades.list.map((u, ui) => (
                        <div key={ui} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', borderBottom: ui < billing.upgrades.list.length-1 ? '1px solid var(--fog)' : 'none' }}>
                          <div>
                          ...
                          {u.mrrDelta > 0 ? <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--forest)', fontWeight: 600 }}>+{'$'+u.mrrDelta+'/mo'}</span> : null}
                        </div>
                      )) : null}
                      {row.key === 'downgrades' ? billing.downgrades.list.map((d, di) => (
                        <div key={di} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', borderBottom: di < billing.downgrades.list.length-1 ? '1px solid var(--fog)' : 'none' }}>
                          <div>
                          ...
                          {d.mrrDelta < 0 ? <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#A32D2D', fontWeight: 600 }}>{'−$'+Math.abs(d.mrrDelta)+'/mo'}</span> : null}
                        </div>
                      )) : null}
                      {row.key === 'cancellations' ? billing.cancelled.list.map((c, ci) => (
                        <div key={ci} style={{ padding: '8px 14px', borderBottom: ci < billing.cancelled.list.length-1 ? '1px solid var(--fog)' : 'none' }}>
                          ...
                          {c.reason ? <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: 'var(--slate)', marginTop: 4, fontStyle: 'italic' }}>{"\""+c.reason+"\""}</div> : null}
                        </div>
                      )) : null}
                    </div>
                  ) : null}
        ...
        <div style={{ background: 'var(--white)', border: '1px solid var(--fog)', borderRadius: 12, padding: '20px 24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {billing.byTenant.map((t, i) => {
              ...
              const depPct   = max > 0 ? (t.deposits / max) * 100 : 0
              const balPct   = max > 0 ? (t.balances / max) * 100 : 0
              const revPct2  = max > 0 ? (t.reviews / max) * 100 : 0
              ...
              return (
                <div key={t.name} style={{ borderBottom: '1px solid var(--fog)', paddingBottom: isOpen ? 8 : 0 }}>
                  <div onClick={() => setCustomerDrill(isOpen ? null : t.name)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4, cursor: 'pointer', padding: '4px 0' }}>
                  ...
                  <div style={{ display: 'flex', height: 6, borderRadius: 3, overflow: 'hidden', background: 'var(--fog)' }}>
                    <RevBar pct={mrrPct}  color="#0A5C46" title={'MRR: $'+t.mrr+'/mo'} />
                    <RevBar pct={depPct}  color="#1A9272" title={'Deposits: $'+t.deposits} />
                    <RevBar pct={balPct}  color="#0F6E56" title={'Balances: $'+t.balances} />
                    <RevBar pct={revPct2} color="#C8952A" title={'Reviews: $'+t.reviews} />
                  </div>
                  <div style={{ display: 'flex', gap: 12, marginTop: 3 }}>
                    ...
                    {t.reviews > 0 ? <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: '#9A6A00' }}>{'reviews $'+t.reviews.toLocaleString()}</span> : null}
                  </div>
                  {isOpen && hasPayments ? (
                    <div style={{ marginTop: 8, background: 'rgba(10,92,70,0.03)', border: '1px solid var(--fog)', borderRadius: 8, overflow: 'hidden' }}>
                      {deposits.map((r, ri) => (
                        <div key={'d'+ri} onClick={() => r.id ? onNavigate('requirements', r.id) : undefined} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 12px', borderBottom: '1px solid var(--fog)', cursor: r.id ? 'pointer' : 'default' }}>
                          <div>
                      ...
                      {balances.map((r, ri) => (
                        <div key={'b'+ri} onClick={() => r.id ? onNavigate('requirements', r.id) : undefined} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 12px', borderBottom: '1px solid var(--fog)', cursor: r.id ? 'pointer' : 'default' }}>
                          <div>
                            <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: 'var(--ink)', fontWeight: 500 }}>{r.title}</div>
                      ...
                      {reviews.map((r, ri) => (
                        <div key={'r'+ri} onClick={() => r.id ? onNavigate('requirements', r.id) : undefined} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 12px', borderBottom: ri < reviews.length-1 ? '1px solid var(--fog)' : 'none', cursor: r.id ? 'pointer' : 'default' }}>
                          <div>
                            <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: 'var(--ink)', fontWeight: 500 }}>{r.title}</div>
