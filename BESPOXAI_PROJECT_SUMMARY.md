...
- New `app/api/admin/tenants/[id]/diagnose/route.ts` (superadmin-gated) and `app/api/partner/tenants/[id]/diagnose/route.ts` (partner-session-gated, any role, read-only) mirror `api/settings/diagnose-connection/route.ts`. **Post-push fix (same day):** first landed as `app/api/admin/tenants/[tenantId]/diagnose/route.ts`, which broke the Vercel build — Next.js disallows two different dynamic-segment names (`[tenantId]` vs the sibling `api/admin/tenants/[id]/route.ts`'s `[id]`) at the same path level. Caught from the failed build, moved to `[id]`, typechecked, re-pushed, confirmed READY on Vercel.
...
**Deploy status (confirmed by Rich):** live on Vercel, READY, no build errors — this whole batch of fixes (BC Auth Mode mirroring + the `[tenantId]`→`[id]` rename + the BC15+ routing fix) is successfully deployed to production.

**Still not tested (open):** the partner-portal and partner-customer side of the BC Auth Mode mirroring — Auth Mode select, Service Account fields, Test Connection, and installer generation from `app/partner/tenants/[id]/page.tsx` — has **not** been manually exercised by Rich yet, only deployed. Same caveat as before for Basic-auth end-to-end against a real container. Both are open verification items, not known bugs — flagging so this doesn't read as "done and tested" when it's really "shipped, not yet walked through."
