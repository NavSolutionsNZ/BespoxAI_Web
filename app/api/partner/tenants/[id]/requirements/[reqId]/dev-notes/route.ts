...
export async function POST(
  ...
  let partnerBrand = 'your BC consultancy'
  try {
    ...
    partnerBrand = account?.brandName ?? account?.name ?? partnerBrand
  } catch (e: any) {
  ...
  // Names
  const consultant = await (prisma as any).user.findFirst({
    where:  { id: session.userId },
    select: { name: true, email: true, preferredName: true },
  })
  const adminName  = consultant?.preferredName ?? consultant?.name ?? 'Your consultant'
  const adminEmail = consultant?.email ?? ''
  ...
  const systemPrompt = `You are a senior Microsoft Dynamics 365 Business Central developer and consultant at ${partnerBrand}.
...
- When drafting consultant notes or scope summaries:
  ...
  • Write in first person as ${adminName} from ${partnerBrand} — you are ghostwriting for them
  • Sign off as ${adminName}, ${partnerBrand}
...
- Do not fabricate specific integration details you are uncertain about — say so explicitly.`
  ...
  const readable = new ReadableStream({
    async start(controller) {
      ...
      controller.close()
      await logAiUsage({ tenantId: requirement.tenantId, requirementId: params.reqId, feature: 'dev_assistant', model: cfg.model, inputTokens, outputTokens })
    },
  })
