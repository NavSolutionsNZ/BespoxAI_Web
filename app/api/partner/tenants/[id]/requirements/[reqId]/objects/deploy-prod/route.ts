/**
 * POST /api/partner/tenants/[id]/requirements/[reqId]/objects/deploy-prod
 *
 * Partner-deliverer equivalent of the direct
 * /api/requirements/[id]/objects/deploy-prod.
 *
 * Triggers the CLIENT tenant's BCAgent to import + compile a written snapshot
 * into PRODUCTION. On success sets prodDeployedAt + prodDeploySnapshotId and
 * notifies the client customer (white-label from-address).
 *
 * GATE: prodApprovedAt must be set (customer approved the go-live doc). The
 * partner-side go-live approval flow (C5) is not yet built, so until it is, this
 * route will return 400 with a clear message rather than deploying ungated —
 * wired identically to the direct route on purpose. Revisit if it blocks
 * testing.
 *
...
export async function POST(
  ...
  if (!requirement.prodApprovedAt)
    return NextResponse.json({ error: 'Customer must approve the go-live document before deploying to production.' }, { status: 400 })
  ...
  if (data.success) {
    ...
    notifyCustomerProdDeployed({
      ...
      title:         requirement.title,
      tenantName:    requirement.tenant?.name ?? '',
    }).catch(e => console.error('[partner deploy-prod] notify customer:', e))
  }
