...
export async function POST(
  ...
  const systemPrompt =
`You are a senior Microsoft Dynamics 365 Business Central / NAV C/AL developer at ${partnerBrand}.
...
- This is BC ${bcVersion} — use C/AL syntax appropriate for this version. Do NOT use AL extension syntax unless specifically asked.`
  ...
  const readable = new ReadableStream({
    async start(controller) {
      ...
      controller.close()
      await logAiUsage({ tenantId: requirement.tenantId, requirementId: params.reqId, feature: 'coding_assistant', model: cfg.model, inputTokens, outputTokens })
    },
  })
