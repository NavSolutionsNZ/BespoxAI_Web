...
export async function POST(
  ...
  try {
    ...
    if (cfg.provider === 'anthropic') {
      ...
      logAiUsage({ tenantId: req_data.tenantId, requirementId: params.id, feature: 'spec_gen', model: cfg.model, inputTokens: d.usage?.input_tokens ?? 0, outputTokens: d.usage?.output_tokens ?? 0 })
    } else {
      ...
      const { logAiUsage } = await import('@/lib/ai-usage')
      logAiUsage({ tenantId: req_data.tenantId, requirementId: params.id, feature: 'spec_gen', model: cfg.model, inputTokens: completion.usage?.prompt_tokens ?? 0, outputTokens: completion.usage?.completion_tokens ?? 0 })
    }
